...
function withCORS(headers, request) {
  headers['accept-ranges'] = 'bytes'; //accept-ranges: bytes
  headers['access-control-allow-origin'] = 'https://corsvktest.herokuapp.com/';
  headers['access-control-allow-methods'] = 'GET, HEAD, OPTIONS';
  headers['access-control-allow-headers'] = 'Range';
  headers['access-control-expose-headers'] = 'Range, Content-Length, Accept-Ranges, Date';
  headers['access-control-allow-credentials'] = 'https://corsvktest.herokuapp.com/';
  return headers;
}
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (url.includes("s=Ss")) {
      var url_temp = url.split("&n=");
      url = url_temp[0];
      var rounds = url_temp[1].replace("&s=Ss", "").replace("&n=", "");
      rounds = parseInt(rounds);
      var i = 0;
      var location_buff = url;
      for (i; i < rounds; i++)
        var location_buff = new Buffer(location_buff, 'base64').toString('ascii');
      var location = parseURL(location_buff);
    }
    else var location = parseURL(url);
